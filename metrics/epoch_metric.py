...
def all_gather(data):
    ...
    print(f'Finish get rank')
    print(data)
    # serialized to a Tensor
    buffer = pickle.dumps(data)
    print(f'Finish buffer at gpu {rank}')
    storage = torch.ByteStorage.from_buffer(buffer)
    print(f'Finish get storage at gpu {rank}')
    tensor = torch.ByteTensor(storage)
    print(f'Finish get type convert at gpu {rank}')
    tensor = tensor.to(device)
    print(f'Finish to device at gpu {rank}')

    print(f'Finish dumps at gpu {rank}')
    ...
    print(f'Start all gather {2}')
    dist.all_gather(tensor_list, tensor)
    print(f'Finish all gather {2}')
...
class EpochMetric:
    ...
    def compute(self) -> Sequence[torch.Tensor]:
        ...
        if _prediction_tensor.ndim > 1:
            nb_classes = _prediction_tensor.shape[1]
        else:
            nb_classes = 1
        if nb_classes > 1:
            _target_tensor = torch.eye(nb_classes)[_target_tensor.view(-1)]
        ...
        if ws > 1:
            ...
            # print(f'Result in metric: {result}')
