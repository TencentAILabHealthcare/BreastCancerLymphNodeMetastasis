...
from models.ordinalreg import AscensionCallback, CutpointsCallback

@dataclass
class OrdCallback(Callback):
    def __post_init__(self):
        self.call_back = AscensionCallback()
        self.cut_call_back = CutpointsCallback()
        super().__init__()

    def on_batch_end(self, batch, logs=None):
        self.call_back(self.trainer.network.link)

    def on_epoch_end(self, epoch, logs=None):
        pass
        print(self.trainer.network.link.cutpoints.data)
        if (epoch + 1) % 10 == 0:
            self.cut_call_back(self.trainer.network.link)
